...
//Procedimiento interno para pasar el contenido de una pila a una cola
procedure pasarPilaACola(var C: Cola; var P: Pila);
var X: TipoElemento;
begin
...
begin
  ...
  Aux1.Crear(Numero,C.SizeQueue);
  Aux2.C.Crear(Numero,C.SizeQueue);
  //Utilizo una pila como estructura auxiliar para lograr invertir los valores
  P.Crear(C.DatoDeLaClave,C.SizeQueue);
  while (not C.EsVacia)do  //Recorro toda la cola origen
  begin
    //Recupero, desencolo en origen y encolo en las dos aux
    X := C.Recuperar;
  ...
  C.InterCambiar(AUX1,false);  //Restauro la cola original
  pasarPilaACola(AUX2.C,P); //Paso el contenido de la pila a la cola
  Result := AUX2;
end;
